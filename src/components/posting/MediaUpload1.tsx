...
import SampleDragDrop from "./SampleDragDrop";
...
export default function MediaUpload({ media, onChange }: MediaUploadProps) {
  ...
  const removeMediaItem = (index: number) => {
    ...
    if (newMedia[index].thumbnailUrl?.startsWith("blob:")) {
  ...
  const handleDragEnd = (event: any) => {
    ...
    const oldIndex = media.findIndex(
      (_, i) => `${media[i].url}-${i}` === active.id
    );
    const newIndex = media.findIndex(
      (_, i) => `${media[i].url}-${i}` === over.id
    );
        ...
        {activeTab === "preview" && media.length > 0 && (
          <div className="space-y-6">
            <div className="relative w-full h-full flex items-center justify-center">
              ...
              >
                <SortableContext
                  items={media.map((item, index) => `${item.url}-${index}`)}
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-2">
                    {media.map((item, index) => (
                      <SortableMediaItem
                        ...
                        item={item}
                        index={index}
                        onRemove={() => removeMediaItem(index)}
                      />
                    ))}
                  </div>
                </SortableContext>
